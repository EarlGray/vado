...
instance RWS.MonadTrans (Measuring) where
  lift = Measuring . RWS.lift

type MeasuringCanvas a = Measuring Canvas.Canvas a
...
getContent request = do
  ...
  Dbg.traceShow content $ fetchImages (manager, request) content
...
blockToBoxes parentEvents parentStyle nodes = do
  ...
  when (y <= maxheight) $ do
    forM_ nodes $ \node -> do
      ...
      Dbg.traceShow node $ case node of
        ...
        TextContent t -> do
          ...
          -- measureLineBreak
        ElementContent _ events style subnodes -> do
    ...
    forM_ nodes $ \node -> do
      ...
      Dbg.traceShow node $ case node of
        ...
        ElementContent _ events style subnodes ->
          inlineToBoxes events (mergeStyles parentStyle style) subnodes

-- | Layout text word-by-word with line-breaking.
textToBoxes :: Events -> Style -> Text -> MeasuringCanvas ()
